...
import {logger} from '../src/config/utils/LoggerUtils'
...
test.describe('Amazon.in Automation Tests', () => {
    test.beforeAll(async () => {
        ...
        logger.info('Logged in to the Amazon account.');
    ...
    test.afterEach(async ({}, testInfo) => {
        if (testInfo.status === 'failed') {
            const screenshotPath = path.join(__dirname, `../src/screenshots/${testInfo.title}.png`);
    ...
    test('Search for Shoes', async () => {
        ...
        logger.info('Searched for Shoes.')
    });
    ...
    test('Apply Filters for Shoes Search', async () => {
        ...
        logger.info('Applied filters for Shoes search.');
    });
    ...
    test('Select Shoe and Verify Details', async () => {
        ...
        await searchPage.applyAdidasFilter();
        await searchPage.applyDeliveryDayFilter();
        logger.info('Selected shoe and verified details.');
    });
    ...
    test('Select and add the product to cart', async () => {
        ...
        logger.info('Selected and added the product to the cart.');
    });
});
